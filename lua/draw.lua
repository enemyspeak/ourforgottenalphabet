...
local clusterClock 		-- this is to keep from triggering an out of bounds instantly
local hideCluster		-- this is to hide the cluster after the out tween is done
...
local function getClusterStars()
	local temp = {}
...
local function finishConstellation()
	...
	tween( 1, clusterTween,{ innerRadius = WIDTH, outerRadius = WIDTH + 10 }, "inBack", setHideCluster)
...
local function updateClock(dt)		-- If you run out of time while drawing, you get to finish your drawing. For now.
	gameElement.gameClock = gameElement.gameClock - dt
	if gameElement.gameClock < 1 then
		gameElement.gameClock = 0
	end
end
...
local function updateCluster(dt)
	if clusterClock > 1 then
		...
		if cluster:getState(x,y) then else
...
local function updateState()
	if scoreFinished and drawFinished then
		...
		local temp1 = gameElement.gameClock + 3 * #constellations
		if temp1 > 100 then 
			temp1 = 99
		end
